...
const Content = ({ blocks }: { blocks: import('../types').ContentBlock[] }) => <div className="lesson-content">{blocks.map((b, i) => {
  switch (b.type) {
    ...
    case 'image': return <figure className="lesson-figure" key={i}><img src={b.src} alt={b.alt} />{b.caption && <figcaption>{b.caption}</figcaption>}</figure>;
...
export default function ChapterStudyPage() {
  ...
  if (!chapter || !subject) return <main className="shell"><section className="card empty"><h1>अध्याय नहीं मिला</h1><Link className="btn" to="/subjects">विषयों पर जाएँ</Link></section></main>;
  ...
  return <main className="shell">
    <div className="page-head">
      ...
      <p>{subject.title} · {pages.length} अध्ययन पृष्ठ · सामग्री मौजूदा पाठ-डेटा से व्यवस्थित</p>
    </div>
    <section className="card lesson-card">
      ...
      <div className="actions"><Link className="btn" to={`/chapters/${chapter.id}`}>← अध्याय पर लौटें</Link></div>
    </section>
  </main>;
}
