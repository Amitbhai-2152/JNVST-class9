...
export const useProgressStore = create<Store>()(persist((set) => ({
  ...initial,
  completeLesson: (id, title) => set((s) => ({ lessonActivity: { ...s.lessonActivity, [id]: { status: "completed", lastAccessed: Date.now() } }, recentlyStudied: [{ id, title, type: "lesson", timestamp: Date.now() }, ...s.recentlyStudied.filter((x) => x.id !== id)].slice(0, 8) })),
  ...
  recordStudy: (id, title, type) => set((s) => ({ recentlyStudied: [{ id, title, type, timestamp: Date.now() }, ...s.recentlyStudied.filter((x) => x.id !== id)].slice(0, 8) })),
