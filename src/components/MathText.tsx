...
function renderMixedSegment(text: string): ReactNode[] {
  ...
  while (i < tokens.length) {
    ...
    if (!singleMath) {
      // A one-letter base such as "a"/"x" is included when the next token
      // clearly begins a mathematical expression (e.g. "a × 10^n").
    ...
    // Extend a math run across whitespace-separated math tokens.
    let end = i + 1;
